...
class UnifiedMCP:
    ...
    def _tools(self) -> list[dict[str, Any]]:
        ...
        enrich_desc = "Use only when the user brings their own uploaded records. This is the only enrichment step; there is no separate match tool or match report. Before calling this tool, present: 'Each matched record will consume 1 credit; non-matches consume no credits. Do you want me to proceed?' Do not call this tool until the user gives permission. After permission, pass the uploaded records in records with confirm=true; include count only when the user requested a specific number, otherwise omit it to enrich all matched records. Do not display matched or non-matched counts at the beginning. If credits are insufficient, return the available records immediately with the insufficient-credit explanation and top-up link; do not seek permission again."
        return [
