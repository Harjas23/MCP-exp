...
class UnifiedMCP:
    ...
    def _tools(self) -> list[dict[str, Any]]:
        ...
        purchase = {"search_id": {"type": "string", "description": "Copy this value exactly from the immediately preceding matching search response. This is internal workflow state; never ask the user to provide it and never invent or transform it."}, "count": {"type": "integer", "minimum": 1, "description": "Use the number requested by the user."}, "confirm": {"type": "boolean", "description": "Set true when the user provides a number of records to reveal; the number is the permission signal. Do not ask a separate yes/no question."}}
        ...
        search_desc = "All filters are optional. Preserve every filter the user explicitly states: map a named city to city, and map a named state such as Ohio to state; do not put a state in city. The search never reveals full records and never calls enrichment. Recommended workflow: MANDATORY RESPONSE ACTION: Display all 10 masked samples from the masked_samples field to the user exactly as returned; do not summarize, omit, or hide them. Then show total matches and aggregate insights. Ask the user: 'How many records would you like to reveal? Each record will consume 1 credit per record.' If the user provides a number, treat it as permission and immediately call the matching purchase tool with the exact search_id returned by this search, the requested count, and confirm=true. Do not ask a separate yes/no confirmation. If the user does not provide a number, do not call the purchase tool."
        ...
        return [
            ...
            _tool("purchase_business", "Workflow: call search_business first. Copy the exact search_id from that response; never ask the user for it, invent it, or transform it. Do not call this tool unless the user provided a record count; the count is the permission signal. Then call with confirm=true. This is the reveal/purchase step, not enrichment. If requested count exceeds available credits, return the available records immediately with the insufficient-credit error and top-up URL; do not ask for permission again. If at least one record is returned, explicitly show credits_deducted and credits_remaining. Freemium users receive the upgrade link and no records.", purchase, ["search_id", "count", "confirm"]),
            _tool("purchase_consumer", "Workflow: call search_consumer first. Copy the exact search_id from that response; never ask the user for it, invent it, or transform it. Do not call this tool unless the user provided a record count; the count is the permission signal. Then call with confirm=true. This is the reveal/purchase step, not enrichment. If requested count exceeds available credits, return the available records immediately with the insufficient-credit error and top-up URL; do not ask for permission again. If at least one record is returned, explicitly show credits_deducted and credits_remaining. Freemium users receive the upgrade link and no records.", purchase, ["search_id", "count", "confirm"]),
            _tool("purchase_contact", "Workflow: call search_contact first. Copy the exact search_id from that response; never ask the user for it, invent it, or transform it. Do not call this tool unless the user provided a record count; the count is the permission signal. Then call with confirm=true. This is the reveal/purchase step, not enrichment. If requested count exceeds available credits, return the available records immediately with the insufficient-credit error and top-up URL; do not ask for permission again. If at least one record is returned, explicitly show credits_deducted and credits_remaining. Freemium users receive the upgrade link and no records.", purchase, ["search_id", "count", "confirm"]),
