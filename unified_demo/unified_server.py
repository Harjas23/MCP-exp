...
class UnifiedMCP:
    ...
    def _tools(self) -> list[dict[str, Any]]:
        ...
        enrich_desc = "Use only when the user brings their own uploaded records. This is the only enrichment step; there is no separate match tool or match report. First call with records to validate and hold the user's rows, but do not display matched or non-matched counts. Present only the permission request, explaining that each matched record consumes 1 credit and non-matches consume no credits. Do not call the second phase until the user gives permission. Then call this same tool with the exact enrichment_id and confirm=true; include count only when the user requested a specific number, otherwise omit it to enrich all matched records. If credits are insufficient, return the available records immediately with the insufficient-credit explanation and top-up link; do not seek permission again."
        return [
