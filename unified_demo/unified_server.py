...
class UnifiedMCP:
    ...
    def enrich(self, state: DemoState, arguments: dict[str, Any], kind: str) -> dict[str, Any]:
        ...
        if not enrichment_id:
            ...
            if not confirm:
                return _text_result({"status": "confirmation_required", "error": "User permission is required before uploaded records can be sent to enrichment. No records were processed.", "permission_message": "Each matched record will consume 1 credit; non-matches consume no credits. Do you want me to proceed?"}, error=True)
    ...
    def _tools(self) -> list[dict[str, Any]]:
        ...
        purchase = {"search_id": {"type": "string", "description": "Copy this value exactly from the immediately preceding matching search response. This is internal workflow state; never request it from the user and never invent or transform it."}, "count": {"type": "integer", "minimum": 1, "description": "Use the number requested by the user."}, "confirm": {"type": "boolean", "description": "Set true when the user provides a number of records to reveal; the number is the permission signal. Do not seek a separate yes/no confirmation."}}
        enrich = {"records": {"type": "array", "maxItems": MAX_BULK_RECORDS, "description": "The user's own uploaded rows parsed by Claude after permission has already been obtained. Do not use this tool for MCP-generated or provider-owned records.", "items": {"type": "object", "additionalProperties": True}}, "count": {"type": "integer", "minimum": 1, "description": "Optional number of records the user authorized; omit to enrich all matched records."}, "confirm": {"type": "boolean", "description": "MANDATORY: set true only after the exact confirmation message was presented and the user explicitly approved. Never call with uploaded records unless this is true."}}
        search_desc = "All filters are optional. Preserve every filter the user explicitly states: map a named city to city, and map a named state such as Ohio to state; do not put a state in city. The search never reveals full records and never calls enrichment. Recommended workflow: MANDATORY RESPONSE ACTION: Display all 10 masked samples from the masked_samples field to the user exactly as returned; do not summarize, omit, or hide them. Then show total matches and aggregate insights. Present exactly: 'How many records would you like to reveal? Each record will consume 1 credit per record.' When the user provides a number or explicit permission, treat it as permission and immediately call the matching purchase tool with the exact search_id returned by this search, the requested count, and confirm=true. Do not seek a separate yes/no confirmation. If the user provides neither a number nor permission, do not call the purchase tool."
        enrich_desc = "Use only when the user brings their own uploaded records. This is the only enrichment step; there is no separate match tool or match report. CREDIT COST: Exactly 1 credit if a record is matched and enriched; 0 credits if it is not matched. MANDATORY CONFIRMATION -- You MUST present this EXACT message to the user before calling this endpoint: 'Each matched record will consume 1 credit; non-matches consume no credits. Do you want me to proceed?' Do NOT call this endpoint without explicit user approval. Only after approval, pass the uploaded records in records with confirm=true; include count only when the user requested a specific number, otherwise omit it to enrich all matched records. Do not display matched or non-matched counts at the beginning. If credits are insufficient, return the available records immediately with the insufficient-credit explanation and top-up link; do not seek permission again."
        return [
            ...
            _tool("purchase_business", "STRICT PRECONDITION: search_business must run first in the current workflow. Copy the exact search_id from that response; never request it from the user, invent it, or transform it. Do not call this tool before search_business or without the user's record count or explicit permission; the count is the permission signal. Call immediately with confirm=true. This is the reveal/purchase step, not enrichment. If requested count exceeds available credits, return the available records immediately with the insufficient-credit error and top-up URL; do not seek permission again. If at least one record is returned, explicitly show credits_deducted and credits_remaining. Freemium users receive the upgrade link and no records.", purchase, ["search_id", "count", "confirm"]),
            _tool("purchase_consumer", "STRICT PRECONDITION: search_consumer must run first in the current workflow. Copy the exact search_id from that response; never request it from the user, invent it, or transform it. Do not call this tool before search_consumer or without the user's record count or explicit permission; the count is the permission signal. Call immediately with confirm=true. This is the reveal/purchase step, not enrichment. If requested count exceeds available credits, return the available records immediately with the insufficient-credit error and top-up URL; do not seek permission again. If at least one record is returned, explicitly show credits_deducted and credits_remaining. Freemium users receive the upgrade link and no records.", purchase, ["search_id", "count", "confirm"]),
            _tool("purchase_contact", "STRICT PRECONDITION: search_contact must run first in the current workflow. Copy the exact search_id from that response; never request it from the user, invent it, or transform it. Do not call this tool before search_contact or without the user's record count or explicit permission; the count is the permission signal. Call immediately with confirm=true. This is the reveal/purchase step, not enrichment. If requested count exceeds available credits, return the available records immediately with the insufficient-credit error and top-up URL; do not seek permission again. If at least one record is returned, explicitly show credits_deducted and credits_remaining. Freemium users receive the upgrade link and no records.", purchase, ["search_id", "count", "confirm"]),
    ...
    def _prompt_text(name: str) -> str:
        prompts = {
            ...
            "business_enrichment": "I uploaded my business records. Before calling the business enrichment tool, obtain my permission by stating that each matched record consumes 1 credit and non-matches consume no credits. Only after permission, pass my uploaded rows with confirm=true and enrich all matched rows without showing match or non-match counts at the beginning.",
            "consumer_enrichment": "I uploaded my consumer records. Before calling the consumer enrichment tool, obtain my permission by stating that each matched record consumes 1 credit and non-matches consume no credits. Only after permission, pass my uploaded rows with confirm=true and enrich all matched rows without showing match or non-match counts at the beginning.",
            "contact_enrichment": "I uploaded my contact records. Before calling the contact enrichment tool, obtain my permission by stating that each matched record consumes 1 credit and non-matches consume no credits. Only after permission, pass my uploaded rows with confirm=true and enrich all contacts, including secondary contacts, without showing match or non-match counts at the beginning.",
