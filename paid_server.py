...
def build_server() -> StdioMCPServer:
    ...
    purchase_props = {
        ...
        "count": {"type": "integer", "minimum": 1, "description": "Use the number requested by the user. For example, 'give me 20 records' means count=20."},
        "confirm": {"type": "boolean", "description": "Set true when the user clearly requests the records, such as 'give me 20'. The search workflow uses one permission checkpoint; do not ask for permission again when the user's count request is clear."},
    }
    tools = [
        tool("search_business", "Search businesses. All filters are optional, including city. Preserve every filter the user explicitly states: when the request says 'in Ohio' or 'in <location>', pass that location as the city argument; omit city only when no location is provided. The search never reveals full records. Recommended workflow: 1) Explicitly display the 10 masked samples to the user. 2) Show the total matches and aggregate insights for the full result set. 3) Ask one combined question for the desired record count and permission. 4) If the user clearly says 'give me 20 records', treat that as count=20 plus permission; do not ask the permission question again. 5) If the user has not provided both a count and permission, do not call purchase_business. Otherwise, call purchase_business with the exact search_id returned by this search, the requested count, and confirm=true.", common_search),
        tool("search_consumer", "Search consumers. All filters are optional. The search never reveals full records. Recommended workflow: 1) Explicitly display the 10 masked samples to the user. 2) Show the total matches and aggregate insights for the full result set. 3) Ask one combined question for the desired record count and permission. 4) If the user clearly says 'give me 20 records', treat that as count=20 plus permission; do not ask the permission question again. 5) If the user has not provided both a count and permission, do not call purchase_consumer. Otherwise, call purchase_consumer with the exact search_id returned by this search, the requested count, and confirm=true.", consumer_search),
        tool("search_contact", "Search contacts. All filters are optional. The search never reveals full records. Recommended workflow: 1) Explicitly display the 10 masked samples to the user. 2) Show the total matches and aggregate insights for the full result set. 3) Ask one combined question for the desired record count and permission. 4) If the user clearly says 'give me 20 records', treat that as count=20 plus permission; do not ask the permission question again. 5) If the user has not provided both a count and permission, do not call purchase_contact. Otherwise, call purchase_contact with the exact search_id returned by this search, the requested count, and confirm=true.", contact_search),
        tool("purchase_business", "Workflow: call search_business first. Copy the exact search_id from that response; never ask the user for it, invent it, or transform it. Do not call this tool before a search. If the user clearly requested a count such as 'give me 20 records', that is the one permission checkpoint—do not ask permission again. Call with count=20 and confirm=true.", purchase_props, ["search_id", "count", "confirm"]),
